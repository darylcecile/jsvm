...
export class ExecutionBudget {
  ...
  checkpoint(stepCost = 1, reason = "execution checkpoint"): void {
    ...
    if (this.maxSteps !== undefined && this.#stepsUsed > this.maxSteps) {
      throw budgetError(VMErrorCode.VMStepsExceededError, "Execution step budget exhausted.", {
        reason,
        path: "maxSteps",
      });
    }
    ...
    if (this.timeLimitMs !== undefined && this.elapsedMs > this.timeLimitMs) {
      throw budgetError(VMErrorCode.VMTimeoutError, "Execution time limit exceeded.", {
        reason,
        path: "timeLimitMs",
      });
    }
...
function budgetError(code: VMErrorCode, message: string, details: Record<string, string>): VMError {
